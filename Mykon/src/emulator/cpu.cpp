...
unsigned int cpu_get_cycles(void) { return c.cycles; }

unsigned short cpu_get_pc() { return c.PC; }
...
//		is_debugged = 0;
  ...
  switch (b) {
    ...
    case 0x27: /* DAA */
      ...
      if (flag_N) {
        if (flag_H) s = (s - 0x06) & 0xFF;
        if (flag_C) s -= 0x60;
      } else {
        if (flag_H || (s & 0xF) > 9) s += 0x06;
        if (flag_C || s > 0x9F) s += 0x60;
      }
