/****************************************************
 * TaskSetup.h
 * 
 * Setup the system tasks
 * 
 ****************************************************/
#ifndef _TASKSETUP_H_
#define _TASKSETUP_H_

/**********************
 * Includes
 **********************/
#include <FreeRTOS.h>
#include <task.h>
#include "cmn/Errors.h"
#include "cmn/ControlTouch.h"

/**********************
 * Defines
 **********************/
#define TASK_MIN_STACK 4096

/**********************
 * Types
 **********************/
enum
{
    tskMED_PRIORITY,
    tskHIGH_PRIORITY,
};

typedef uint32_t ntfy_app_t8;
enum
{
    /* Task Behavior */
    NTFY_NONE,
    NTFY_SETUP,
    NTFY_STRT,
    NTFY_SUSPEND,

    /* IO Notifications ( Joystick ) */
    NTFY_IO_JYSTCK_UP,
    NTFY_IO_JYSTCK_DOWN,
    NTFY_IO_JYSTCK_LEFT,
    NTFY_IO_JYSTCK_RIGHT,
    NTFY_IO_JYSTCK_CENTER,
    NTFY_IO_BTN_JYSTCK,
    NTFY_IO_BTN_JYSTCK_RELEASE,

    /* IO Notifications ( Buttons ) */
    NTFY_IO_BTN_A,
    NTFY_IO_BTN_A_RELEASE,
    NTFY_IO_BTN_X,
    NTFY_IO_BTN_X_RELEASE,
    NTFY_IO_BTN_Y,
    NTFY_IO_BTN_Y_RELEASE,
    NTFY_IO_BTN_B,
    NTFY_IO_BTN_B_RELEASE,
    NTFY_IO_BTN_HOME,
    NTFY_IO_BTN_HOME_RELEASE,

    NTFY_IO_FIRST = NTFY_IO_JYSTCK_UP,
    NTFY_IO_LAST = NTFY_IO_BTN_HOME_RELEASE,
    NTFY_IO_COUNT = NTFY_IO_LAST - NTFY_IO_FIRST + 1,
};

/* All User and Kernel Apps */
typedef uint8_t app_list_t8;
enum
{
    /* User Apps*/
    APP_TICTACTOE,
    APP_SLOTMACHINE,
    APP_TOUCHTIME,
    APP_PONG,
    APP_GAMEBOY,
    APP_USER_LAST = APP_GAMEBOY,

    /* Kernel Apps */
    APP_MYKON,
    APP_DEV,
    APP_IO,
    APP_AUDIO,
    APP_KRNL_LAST = APP_AUDIO,

    APP_COUNT_TOTAL = APP_KRNL_LAST + 1,
    APP_COUNT_USER = APP_USER_LAST + 1,
    APP_COUNT_KRNL = APP_COUNT_TOTAL - APP_COUNT_USER,
    
};

/* Type alias */
using TaskSetupFunction = void (*)(void);

typedef struct 
{
    TaskHandle_t       tsk_hndl;
    TaskSetupFunction  setup_fnctn;
    app_list_t8        app_sbscrptn;

} Mykon_Hook_s;

/**********************
 * Function Prototypes
 **********************/
void GetMykonHooks( Mykon_Hook_s hooks[ APP_COUNT_TOTAL ] );
void MykonResume( );

/**********************
 * Variables
 **********************/

/**********************
 * Functions
 **********************/
mk_err_t Init_Task_Mykon( );
mk_err_t Init_Task_Dev( );
mk_err_t Init_Task_IO( );
mk_err_t Init_Task_Audio( );
mk_err_t Init_Task_TicTacToe( );
mk_err_t Init_Task_SlotMachine( );
mk_err_t Init_Task_TouchTime( );
mk_err_t Init_Task_Pong( );
mk_err_t Init_Task_GameBoy( );

#endif